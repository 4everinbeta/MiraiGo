...
* **TDD - Response Fields Mapping**
  * - [ ] Task: Write failing pytest unit/integration tests in `src/tests/providers/test_duffel.py` verifying that all expected fields (`price_minor`, `currency_code`, `duration_minutes`, `stops_count`) are parsed from the Duffel response and mapped to `FlightSearchResult`.
  * - [ ] Task: Update the response mapping in `src/app/scrapers/duffel.py` (or the Duffel provider) to match the fields correctly. Verify that unit tests pass.
