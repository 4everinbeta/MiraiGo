...
* **TDD - Intent Parsing Refinement**
  * - [ ] Task: Write failing pytest unit tests in `src/tests/nlp/test_intent_refinement.py` asserting that queries like "Flights in July from Denver" correctly extract Denver as the origin rather than the destination, and that relative/month queries resolve correctly.
