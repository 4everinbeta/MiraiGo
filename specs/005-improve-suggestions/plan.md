...
**Primary Dependencies**: FastAPI, Pydantic, existing SearchService clarification flow, existing ranking/suggestion models, Jest/Playwright test suites
...
**Constraints**: Must keep natural-language-first flow, must not regress clarification loop stability, must not introduce provider coupling into suggestion ranking logic
