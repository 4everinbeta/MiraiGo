...
test.describe('Clarification flow', () => {
  test('handles iterative follow-up answers, recap edits, and continue CTA handoff', async ({
    ...
    await page.route('**/api/v1/orchestrator/turn', async (route) => {
      const turnRequest = route.request().postDataJSON() as Record<string, unknown>
      const body = (turnRequest.search_payload || {}) as Record<string, unknown>
      ...
      if (searchTurn === 1) {
        ...
        await route.fulfill({
          json: {
            ...
            markdown: 'How many days should this trip be?',
            open_questions: ['How many days should this trip be?'],
            candidate_destinations: [],
            packages: [],
            itineraries: [],
            disclaimers: [],
            executed_agents: ['intake'],
            search_response
          }
        })
        return
      ...
      if (searchTurn === 2) {
        ...
        await route.fulfill({
          json: {
            session_id: 'session-1',
            response_type: 'questions',
            markdown: 'What budget should we target?',
            open_questions: ['What budget should we target?'],
            candidate_destinations: [],
            packages: [],
            itineraries: [],
            disclaimers: [],
    ...
    await page.getByLabel(/travel prompt/i).fill('Warm trip in June with moderate spend')
    ...
    await expect(page.getByText(/seoul stay options/i).first()).toBeVisible()
